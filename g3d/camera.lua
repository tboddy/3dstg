...
function camera.getLookVector()
    ...
    local vy = camera.target[3] - camera.position[3]
    local vz = camera.target[2] - camera.position[2]
...
function camera.lookInDirection(x,y,z, directionTowards,pitchTowards)
    ...
    camera.position[3] = y or camera.position[3]
    camera.position[2] = z or camera.position[2]
    ...
    camera.target[3] = camera.position[3]+math.sin(fpsController.direction)*cosPitch
...
function camera.firstPersonMovement(dt)
    ...
    if moveX ~= 0 or moveY ~= 0 then
        ...
        camera.position[3] = camera.position[3] + math.sin(fpsController.direction + angle) * speed * dt
        cameraMoved = true
    end
...
function camera.firstPersonLook(dx,dy)
    ...
    camera.lookInDirection(camera.position[1],camera.position[3],camera.position[2], fpsController.direction,fpsController.pitch)
end
