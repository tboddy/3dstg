...
return {
	...
	load = function(self)
		...
		local propAngle, treeLine, bushLine, grassLine = 0, 42, 38.5, 5
		local angleOffset
		...
		for i = 1, 32 do
			...
			-- some grass
			grassOffset, angleOffset = grassLine + math.random(18, 31), offsetReturner()
		...
		for i = 1, self.tileCount do
			if self.tiles[i].active then
				if self.tiles[i].sky then
					self.tiles[1].model:setTranslation(g3d.camera.position[1], g3d.camera.position[2] - 50, g3d.camera.position[3])
